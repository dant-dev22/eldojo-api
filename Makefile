.PHONY: help install venv start start-no-reload health db-check docs clean lint-check db-reset db-migrate db-seed db-teardown db-status
...
help:
	...
	@echo "  make db-migrate      - Apply Alembic head + all 10 manual migrations (idempotent)"
	@echo "  make db-seed         - Seed master data + 6 demo seeds (40 students, attendance)"
...
db-migrate:
	...
	@echo "[eldojo-api-db] step 2/2 — applying 10 manual migrations (idempotent runner)..."
	@$(VENV_PY) "$(PROJECT_ROOT)scripts_run_sql_files.py" \
		"$$($(VENV_PY) -c 'from app.core.config import settings; print(settings.database_url)')" \
		migration $(addprefix $(PROJECT_ROOT),$(MANUAL_MIGRATIONS))
...
db-seed:
	...
	@echo "[eldojo-api-db] step 2/2 — 6 demo seeds (classes, 40 students, enrollments, payments, attendance)"
	@$(VENV_PY) "$(PROJECT_ROOT)scripts_run_sql_files.py" \
		"$$($(VENV_PY) -c 'from app.core.config import settings; print(settings.database_url)')" \
		seed $(addprefix $(PROJECT_ROOT),$(MANUAL_SEEDS))
